...
void convolution( cv::Mat& gray, const cv::Mat& kernel, cv::Mat& dst )
{
	...
	/*  Working area - begin */
	double normalization = 1.f / (cv::sum(abs(kernel)).val[0] + 0.000000001);
		...
		for (int j = 1; j < gray.cols - 1; j++)
		{
			...
			for (int k = 2; k >= 0; k--)
			{
				for (int l = 2; l >= 0; l--)
				{
					...
					if (x >= 0 && x < gray.rows && y >= 0 && y < gray.cols)
					{
						dst.at<float>(i, j) += gray.at<uchar>(x, y) * kernel.at<float>(k, l);
					}	
				}
			}

			dst.at<float>(i, j) *= normalization;
		}
	}
	/*  Working area - end */
...
void geometricalTransform( const cv::Mat& src, cv::Mat& dst, const cv::Mat& transformation )
{
	...
	*/
	...
	/*  Working area - begin */
	// walk trough image as a point
	cv::Vec3f p(0, 0, 1);
	for (; p[1] < dst.rows; p[1]++)
	{
		for (; p[0] < dst.cols; p[0]++)
		{
			// get transformed position of a point p by inverse trasformation matrix
			cv::Vec3f t((float*) cv::Mat(T * cv::Mat(p)).data);

			t[1] = cvRound(t[1]);
			t[0] = cvRound(t[0]);

			// ignore out of border
			if (t[1] >= 0 && t[1] < src.rows 
				&& t[0] >= 0 && t[0] < src.cols)
			{
				dst.at<uchar>(p[1], p[0]) = src.at<uchar>(t[1], t[0]);
			}
		}
		p[0] = 0;
	}
