...
void passFilter( const cv::Mat& src, cv::Mat& dst, int limit_frequency, int flag, cv::Mat * spektrum = NULL )
{
	...
	*/
	...
	{
		for (int i = spectrumCplx.cols / 2; i < spectrumCplx.cols / 2; i++)
		{
			for (int j = spectrumCplx.rows / 2; j < spectrumCplx.rows / 2; j++)
			{
				spectrumCplx.at<cv::Vec2f>(i, j)[0] = 0;
				spectrumCplx.at<cv::Vec2f>(i, j)[1] = 0;
			}
		}

		/*cv::Mat(cv::Mat::zeros(cv::Size(limit_frequency, limit_frequency), spectrumCplx.type()))
			.copyTo(spectrumCplx(cv::Rect(
				spectrumCplx.cols / 2,
				spectrumCplx.rows / 2,
				spectrumCplx.cols / 2 + limit_frequency,
				spectrumCplx.rows / 2 + limit_frequency
			)));*/
	}
	...
	cv::normalize( srcPadded, srcPadded, 0, 1, cv::NORM_MINMAX );
...
cv::Mat spectrumMagnitude( cv::Mat & specCplx )
{
	...
	cv::normalize( specMag, specMag, 0, 1, cv::NORM_MINMAX );
	return specMag;
}
